...
\subsection{การเข้ารหัสคำและการฝังตำแหน่ง}\label{subsec:embedding-positional}

Transformer จำเป็นต้องแปลงโทเคนเชิงสัญลักษณ์ให้เป็นเวกเตอร์เชิงตัวเลขเพื่อให้ดำเนินการเชิงเส้นได้ สององค์ประกอบที่สำคัญคือ Embedding Matrix และ Positional Encoding

\subsubsection{Token Embeddings}

ให้ $E \in \mathbb{R}^{V \times d_{\text{model}}}$ เป็นเมทริกซ์ embedding ที่เรียนรู้ได้ ($V$ คือขนาดคลังคำศัพท์, $d_{\text{model}}$ คือมิติของเวกเตอร์) โทเคนลำดับที่ $t$ จะถูกแม็ปเป็นเวกเตอร์
\[
\mathbf{x}_t = E[t] \in \mathbb{R}^{d_{\text{model}}}
\]
เวกเตอร์เหล่านี้อาศัยโครงสร้างของปริภูมิเวกเตอร์ ทำให้การดำเนินการเชิงเส้น (เช่น การลบแปลความหมาย หรือการคูณเมทริกซ์) สามารถจับความสัมพันธ์เชิงความหมายได้ ตัวอย่างที่พบได้บ่อย คือ $\text{king} - \text{man} + \text{woman} \approx \text{queen}$

\subsubsection{Positional Encoding}\label{subsubsec:pos-encoding}

เนื่องจากสถาปัตยกรรม Transformer ไม่มีการวนซ้ำหรือคอนโวลูชัน โมเดลจึงต้องได้รับข้อมูลตำแหน่งเพื่อรับรู้ลำดับของโทเคน ข้อมูลนี้ถูกเพิ่มเข้าไปในเวกเตอร์ embedding แบบองค์ประกอบต่อองค์ประกอบเพื่อให้เกิดการแทนเชิงเส้นที่มีทั้งข้อมูลความหมายและลำดับ

\paragraph{Sinusoidal Positional Encoding}

Positional Encoding แบบไซน์และโคไซน์นิยมใช้เพราะเป็นฟังก์ชันเชิงกำหนด (deterministic) ที่รองรับความยาวลำดับไม่จำกัด และยังคงใช้การดำเนินการเชิงเส้นในการรวมกับ embedding สูตรมาตรฐานคือ
\[
...
\subsection{กลไก Self-Attention}\label{subsec:self-attention}

Self-Attention เป็นแกนหลักที่ทำให้ Transformer สามารถ ``โฟกัส'' โทเคนที่เกี่ยวข้องกันภายในประโยคเดียวกันได้ ข้อมูลอินพุตถูกจัดเก็บในเมทริกซ์ $X \in \mathbb{R}^{n \times d_{\text{model}}}$ โดย $n$ คือความยาวลำดับ จากนั้นสร้างเวกเตอร์ Query, Key, Value ผ่านการฉายเชิงเส้น
\[
...
ในงานทำนายโทเคนถัดไป (autoregressive) จำเป็นต้องป้องกันไม่ให้โทเคนมองเห็นข้อมูลอนาคต จึงใช้เมทริกซ์หน้ากาก $M \in \mathbb{R}^{n \times n}$ ที่มีค่า $0$ เมื่อตำแหน่งอนุญาตให้มองเห็น และ $-\infty$ เมื่อไม่อนุญาต กลไกจึงปรับเป็น
\[
...
\subsection{กลไกแบบ Multi-Head Attention}\label{subsec:multi-head}

Self-Attention เพียงหัวเดียวอาจจับได้เพียงบางแง่มุมของความสัมพันธ์ โครงสร้าง Multi-Head จึงทำซ้ำ Self-Attention หลายชุดใน subspace ที่แตกต่างกัน จากนั้นเชื่อมผลลัพธ์กลับเข้าด้วยกัน
\[
...
\subsection{โครงสร้าง Transformer Block และ Encoder--Decoder}\label{subsec:transformer-block}

แต่ละ Transformer Block ประกอบด้วยลำดับของชั้นเชิงเส้นและไม่เชิงเส้นที่ผูกกับแนวคิด Residual Connection และ Layer Normalization เพื่อรักษาเสถียรภาพการเรียนรู้

\subsubsection{Self-Attention Layer}

อินพุต $\mathbf{x}$ จะถูกส่งผ่าน Multi-Head Self-Attention เพื่อรับเวกเตอร์บริบท $\mathbf{c}$ แล้วใช้ Residual Connection และ Layer Normalization
\[
...
แต่ละโทเคนผ่าน FFN สองชั้นที่ใช้ฟังก์ชันไม่เชิงเส้น (เช่น ReLU หรือ GELU) ทำงานแบบจุดต่อจุด
\[
...
สถาปัตยกรรมดั้งเดิมของ Transformer มีสองส่วน
\begin{enumerate}
